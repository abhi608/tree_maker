...
string='';
var data=fs.readFileSync("second_level.json");
...
// 	string+=' events[0].name == "'+events[i]+'" && events[1].name == "'+events[j]+'" ';
// 	var data3=' ) {++state.count;}for (var i = 0; i < events.length - 2; ++i) { if (events[i].name == "Sign Up-V2" && ';
// 	string+=data3.toString();
// 	string+=' events[i+1].name == "'+events[i]+'" && events[i+2].name == "'+events[j]+'" ';
...
// 	    method: 'POST'
// 	};
// 	var obj={first : i, second : j, params : params};
// 	arr.push(obj);
// 	string=temp_string1;
// }
// }
...
for(var i=0;i<data.length;i++){
	if(data[i].percent>1){
		for(var j=0;j<events.length;j++){
			string+=' events[0].name == "'+data[i].second+'" && events[1].name == "'+events[j]+'" ';
			var data3=' ) {++state.count;}for (var i = 0; i < events.length - 2; ++i) { if (events[i].name == "Sign Up-V2" && ';
			string+=data3.toString();
			string+=' events[i+1].name == "'+data[i].second+'" && events[i+2].name == "'+events[j]+'" ';
			var data4=') {++state.count;}}if (events[events.length - 1].name == "Sign Up-V2") {state.last_login = true;}a=events[0].name;}return state;}).map(function(event) { return event.value.count }).reduce(mixpanel.reducer.sum());}';
			string+=data4.toString();
			var params = {
	    		url: 'https://9ec92a54f32d495f769596edfc8c601f@mixpanel.com/api/2.0/jql', //URL to hit
...
async.mapLimit(arr, 50, test, function(err,results){
	...
	//console.log(results);
		...
		fs.writeFile(path.join(__dirname +'/third_level.json'), myjson, function(err) {
			if (err) {
...
function test(arr, cb){
	...
	request(params, function(error, response, body){
	    if(error) {
	        ...
	        var result = JSON.parse(body);
	        //if (result[0]!=0) {
	        	console.log('Sign Up-V2->'+data[arr.first].second+'->'+events[arr.second]+': '+result[0]);
	        	var obj={second: data[arr.first].second, third: events[arr.second], fourth: null, fifth:null, value: result[0]};
	        //}
	    	return cb(null, obj);
	    }
	});
