...
var data=fs.readFileSync("fourth_level.json");
...
for(var i=0;i<data.length;i++){
	if(data[i].percent>1){
		for(var k=0;k<events.length;k++){
			//console.log("entered");
			string+=' events[0].name == "'+data[i].second+'" && events[1].name == "'+data[i].third+'" && events[2].name == "'+data[i].fourth+'" && events[3].name == "'+events[k]+'" ';
			var data3=' ) {++state.count;}for (var i = 0; i < events.length - 4; ++i) { if (events[i].name == "Sign Up-V2" && ';
			string+=data3.toString();
			string+=' events[i+1].name == "'+data[i].second+'" && events[i+2].name == "'+data[i].third+'" && events[i+3].name == "'+data[i].fourth+'" && events[i+4].name == "'+events[k]+'" ';
			var data4=') {++state.count;}}if (events[events.length - 1].name == "Sign Up-V2") {state.last_login = true;}a=events[0].name;}return state;}).map(function(event) { return event.value.count }).reduce(mixpanel.reducer.sum());}';
			string+=data4.toString();
			var params = {
	    		url: 'https://9ec92a54f32d495f769596edfc8c601f@mixpanel.com/api/2.0/jql', //URL to hit
			...
			var obj={first : i, second : i, third: i, fourth:k, params : params};
			arr.push(obj);
			string=temp_string1;
		}
	}
...
async.mapLimit(arr, 50, test, function(err,results){
	if(err){
		...
		fs.writeFile(path.join(__dirname +'/fifth_level.json'), myjson, function(err) {
			if (err) {
...
function test(arr, cb){
	...
	request(params, function(error, response, body){
	    if(error) {
	        ...
	        var result = JSON.parse(body);
	        //if (result[0]!=0) {
	        	console.log('Sign Up-V2->'+data[arr.first].second+'->'+data[arr.second].third+'->'+data[arr.third].fourth+'->'+events[arr.fourth]+': '+result[0]);
	        	var obj={second: data[arr.first].second, third: data[arr.second].third, fourth: data[arr.third].fourth, fifth: events[arr.fourth], value: result[0]};
	        //}
	    	return cb(null, obj);
	    }
	});
